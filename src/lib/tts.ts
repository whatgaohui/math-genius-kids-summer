...
async function hasUsableVoices(lang: string): Promise<boolean> {
  ...
  try {
    ...
    if (!hasMatchingVoice) {
      // 没有目标语言的系统声音（常见：国产系统无英语语音包）。
      // 不能返回 true，否则会用错误语言的声线朗读，听感近似无声。
...
function speakWithLocalAudio(text: string, lang: string, speed: number): Promise<boolean> {
  ...
  return new Promise((resolve) => {
    ...
    // 安全兜底：15 秒内没结束（卡死）按当前状态收尾
    setTimeout(() => {
      if (!settled) finish(audio.currentTime > 0);
    }, 15000);
 ...
 *
 * 2. Pre-generated local audio (public/audio/en/) — for browser
 *    - English vocabulary mp3s generated by `npm run gen:tts-audio`
 *    - No dependency on system TTS voices (works on phones without
 *      English speech packs, where Web Speech API stays silent)
 *
 * 3. Web Speech API (window.speechSynthesis) — for browser
 ...
 *
 * 4. Backend TTS API (/api/tts) — for browser fallback
 *    - Only works when Next.js server is running (not in APK)
 *    - Uses z-ai-web-dev-sdk for high-quality TTS
 *
 * 5. Silent resolve — if nothing works, don't crash the app
 */
export function speakWithAPI(
  ...
  // ── PRIORITY 2: 预生成本地音频（英语词汇，不依赖系统 TTS 声音）──
  return speakWithLocalAudio(text, lang, speed).then((handledByLocalAudio) => {
    if (handledByLocalAudio) return Promise.resolve();

    // ── PRIORITY 3: Web Speech API (browser only) ──
